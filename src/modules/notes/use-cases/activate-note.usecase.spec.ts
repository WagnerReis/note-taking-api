import { UniqueEntityId } from '@/core/entities/unique-entity-id';
import { BadRequestError } from '@/core/errors/bad-request.error';
import { ConflictError } from '@/core/errors/conflict.error';
import { InternalServerError } from '@/core/errors/internal-server.error';
import { User } from '@/modules/users/entities/user.entity';
import { makeNote } from 'test/factories/make-note.factory';
import { makeUser } from 'test/factories/make-user.factory';
import { InMemoryNotesRepository } from 'test/repositories/in-memory-note.repository';
import { InMemoryUsersRepository } from 'test/repositories/in-memory-user.repository';
import { Note, StatusEnum } from '../entities/note.entity';
import {
  ActivateNoteCommand,
  ActivateNoteUseCase,
} from './active-note.usecase';
import { NoteNotFoundError } from './errors/note-not-found.error';

let inMemoryNotesRepository: InMemoryNotesRepository;
let inMemoryUsersRepository: InMemoryUsersRepository;
let SUT: ActivateNoteUseCase;

describe('Activate note use case', () => {
  beforeEach(() => {
    inMemoryNotesRepository = new InMemoryNotesRepository();
    inMemoryUsersRepository = new InMemoryUsersRepository();
    SUT = new ActivateNoteUseCase(inMemoryNotesRepository);
  });

  it('should be able to activate a note', async () => {
    const user = User.create({
      email: 'test@test.com',
      name: 'test',
      password: 'test',
    });

    await inMemoryUsersRepository.create(user);

    const note = Note.create({
      title: 'test',
      content: 'test content',
      status: StatusEnum.ARCHIVED,
      tags: ['test', 'test2'],
      userId: user.id,
    });

    await inMemoryNotesRepository.create(note);

    const command = new ActivateNoteCommand(
      note.id.toString(),
      user.id.toString(),
    );

    const result = await SUT.execute(command);

    expect(result.isRight()).toBe(true);
    expect(inMemoryNotesRepository.notes).toHaveLength(1);
    expect(inMemoryNotesRepository.notes[0].status).toBe(StatusEnum.ACTIVE);
  });

  it('should not be able to activate a non-existing note', async () => {
    const command = new ActivateNoteCommand(
      'non-existing-note-id',
      'non-existing-user-id',
    );

    const result = await SUT.execute(command);

    expect(result.isLeft()).toBe(true);
    expect(result.value).toBeInstanceOf(NoteNotFoundError);
  });

  it('should not be able to activate a note from a non-existing user', async () => {
    const note = Note.create({
      title: 'test',
      content: 'test content',
      status: StatusEnum.ARCHIVED,
      tags: ['test', 'test2'],
      userId: new UniqueEntityId('existing-user-id'),
    });

    await inMemoryNotesRepository.create(note);

    const command = new ActivateNoteCommand(
      note.id.toString(),
      'non-existing-user-id',
    );

    const result = await SUT.execute(command);

    expect(result.isLeft()).toBe(true);
    expect(result.value).toBeInstanceOf(NoteNotFoundError);
  });

  it('should throw a BadRequestError if noteId is not provided', async () => {
    const command = new ActivateNoteCommand('', '');

    const result = await SUT.execute(command);

    expect(result.isLeft()).toBe(true);
    expect(result.value).toBeInstanceOf(BadRequestError);
  });

  it('should throw a BadRequestError if userId is not provided', async () => {
    const command = new ActivateNoteCommand('1231312', '');

    const result = await SUT.execute(command);

    expect(result.isLeft()).toBe(true);
    expect(result.value).toBeInstanceOf(BadRequestError);
  });

  it('should throw a ConflictError if note is already activated', async () => {
    const note = Note.create({
      title: 'test',
      content: 'test content',
      status: StatusEnum.ACTIVE,
      tags: ['test', 'test2'],
      userId: new UniqueEntityId('existing-user-id'),
    });

    note.activate();

    await inMemoryNotesRepository.create(note);

    const command = new ActivateNoteCommand(
      note.id.toString(),
      note.userId.toString(),
    );

    const result = await SUT.execute(command);

    expect(result.isLeft()).toBe(true);
    expect(result.value).toBeInstanceOf(ConflictError);
  });

  it('should throw InternalServerError if repository update fails', async () => {
    const user = makeUser();
    await inMemoryUsersRepository.create(user);

    const note = makeNote({ userId: user.id, status: StatusEnum.ARCHIVED });
    await inMemoryNotesRepository.create(note);

    vi.spyOn(inMemoryNotesRepository, 'update').mockRejectedValueOnce(
      new InternalServerError('Database connection failed'),
    );

    const command = new ActivateNoteCommand(
      note.id.toString(),
      user.id.toString(),
    );

    const result = await SUT.execute(command);

    expect(result.isLeft()).toBe(true);
    expect(result.value).toBeInstanceOf(InternalServerError);
    if (result.value instanceof InternalServerError) {
      expect(result.value.message).toBe(
        'Failed to activate note: Error: Database connection failed',
      );
    }
  });
});
